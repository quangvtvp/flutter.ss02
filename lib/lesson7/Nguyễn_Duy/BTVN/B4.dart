...
class TiktokVideo {
  ...
  void displayInfo() {
    print(
        '$title | Views: $views | Likes: $likes | Engagement: ${engagement.toStringAsFixed(4)}');
  }
}
...
void main() {
  ...
  for (int i = 0; i < n; i++) {
    ...
    videos.add(TiktokVideo(title, views, likes, i));
  }
  videos.sort((a, b) {
  ...
  List<TiktokVideo> bucket2 =
      videos.where((v) => v.views >= 10000 && v.views <= 100000).toList();
  ...
  if (videos.isNotEmpty) {
    TiktokVideo longest =
        videos.reduce((a, b) => a.title.length > b.title.length ? a : b);
    TiktokVideo shortest =
        videos.reduce((a, b) => a.title.length < b.title.length ? a : b);
