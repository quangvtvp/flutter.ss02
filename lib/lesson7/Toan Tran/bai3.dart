...
void main() {
  ...
  //Loại bỏ trùng (giữ năm lớn nhất)
  ...
  for (int i = 0; i < (searchResult.length < 5 ? searchResult.length : 5); i++) {
    searchResult[i].displayInfo();
  }

  //Thống kê tổng số trang theo tác giả
  Map<String, int> pagesByAuthor = {};
  for (var b in books) {
    pagesByAuthor[b.author] = (pagesByAuthor[b.author] ?? 0) + b.pages;
  }

  // Tìm tác giả có tổng số trang cao nhất
  String topAuthor = '';
  int maxPages = 0;
